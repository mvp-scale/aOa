...
def _trigger_domain_learning_if_needed(project_id: str, intent_index: 'IntentIndex' = None):
    """
    Event-driven learning triggered on every intent (Session 70).

    Called from record_intent() - async, non-blocking.

    Every prompt:
    - ED-01: Extract bigrams from conversation
    - ED-02: Track file hits from intent records

    Every 25 prompts:
    - Rebalance keywords (cohit-based assignment)

    Every 100 prompts:
    - Autotune (domain ranking, decay, prune)
    """
