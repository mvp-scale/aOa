...
cmd_grep() {
    ...
    _grep_flag() {
        case "$1" in
            ...
            c) count_only=true ;;
            q) quiet=true ;;
            E) shift; cmd_egrep "$@"; exit $? ;;
            r|R|n|H|F|l) ;; # No-ops: always recursive, line numbers, filenames, literal
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            -e|--regexp)
                ...
                ;;
            --include|--include=*)
                # Unix grep --include="*.py" → aOa file filter
                if [[ "$1" == --include=* ]]; then
            ...
            -[a-zA-Z][a-zA-Z]*)
                ...
                if [ -n "$unknown_in_combo" ]; then
                    echo -e "${DIM}aOa grep searches indexed symbols (exact token match).${NC}"
                    echo -e "${DIM}Unsupported flag: -${unknown_in_combo}. Supported: -i -w -c -q -e -E -a${NC}"
                    return 1
                fi
                ;;
            -*)
                echo -e "${DIM}aOa grep searches indexed symbols (exact token match).${NC}"
                echo -e "${DIM}Unsupported: $1${NC}"
                echo -e "${DIM}Supported flags: -i -w -c -q -e -E -a --since --include${NC}"
                echo -e "${DIM}For regex/substring: aoa egrep \"pattern\"${NC}"
                return 1
                ;;
            *)
