...
cmd_cc() {
    ...
    case "$subcmd" in
        ...
        sessions|s)
            ...
            # Dividers at fixed positions: 32, 59, 77, 95
            # SESSION=31 │ TOKENS=26 │ PERF=17 │ MODELS=17 │ TOOLS=27

            # Headers - using exact character counts
            echo -e "${DIM}SESSION                        ${DIM}│${DIM} TOKENS                    ${DIM}│${DIM} PERF            ${DIM}│${DIM} MODELS          ${DIM}│${DIM} TOOLS${NC}"
            echo -e "${DIM}START       DUR     P      T   ${DIM}│${DIM}    IN     OUT        C    ${DIM}│${DIM}   T/S      C%   ${DIM}│${DIM}   O    S    H   ${DIM}│${DIM}  B    R    E    W    T    M${NC}"
            ...
            echo "$result" | jq -r '.sessions[] | [
                ...
                (.prompt_count // 0),
                (.turn_count // 0),
                (if (.input_tokens // 0) >= 1000000 then "\(((.input_tokens // 0) / 1000000 * 10 | floor) / 10)M" elif (.input_tokens // 0) >= 1000 then "\(((.input_tokens // 0) / 1000) | floor)k" else "\(.input_tokens // 0)" end),
                (if (.output_tokens // 0) >= 1000000 then "\(((.output_tokens // 0) / 1000000 * 10 | floor) / 10)M" elif (.output_tokens // 0) >= 1000 then "\(((.output_tokens // 0) / 1000) | floor)k" else "\(.output_tokens // 0)" end),
                (if ((.cache_hit // 0) * (.input_tokens // 0) / 100) >= 1000000 then "\((((.cache_hit // 0) * (.input_tokens // 0) / 100) / 1000000 * 10 | floor) / 10)M" elif ((.cache_hit // 0) * (.input_tokens // 0) / 100) >= 1000 then "\((((.cache_hit // 0) * (.input_tokens // 0) / 100) / 1000) | floor)k" else "\(((.cache_hit // 0) * (.input_tokens // 0) / 100) | floor)" end),
                (if (.velocity // 0) >= 1000 then "\(((.velocity // 0) / 1000 * 10 | floor) / 10)k" else "\((.velocity // 0) | floor)" end),
                "\(.cache_hit // 0)%",
                (.models.O // 0),
                (.models.S // 0),
                (.models.H // 0),
                ...
                (.tools.T // 0),
                (.tools.M // 0)
            ] | @tsv' 2>/dev/null | head -"$limit" | while IFS=$'\t' read -r date dur p t inp out c ts cp o s h bb rr ee ww tt mm; do
                # Build each section as fixed-width string then pad to exact section width
                local sec1=$(printf "%-10s %6s %5s %6s" "$date" "$dur" "$p" "$t")
                local sec2=$(printf "%5s %6s %10s" "$inp" "$out" "$c")
                local sec3=$(printf "%6s %7s" "$ts" "$cp")
                local sec4=$(printf "%4s %4s %4s" "$o" "$s" "$h")
                local sec5=$(printf "%4s %4s %4s %4s %4s %4s" "$bb" "$rr" "$ee" "$ww" "$tt" "$mm")

                # Pad each section to EXACT width: 31 │ 27 │ 17 │ 17 │ rest
                sec1=$(printf "%-31s" "$sec1")
                sec2=$(printf "%-27s" "$sec2")
                sec3=$(printf "%-17s" "$sec3")
            ...
            # Legend
            echo -e "${DIM}───────────────────────────────${DIM}┼${DIM}───────────────────────────${DIM}┼${DIM}─────────────────${DIM}┼${DIM}─────────────────${DIM}┼${DIM}───────────────────────────${NC}"
            echo -e "${DIM}P=Prompts  T=Turns             ${DIM}│${DIM} IN=Input  OUT=Output      ${DIM}│${DIM} T/S=Opus tok/s  ${DIM}│${DIM} O=Opus          ${DIM}│${DIM} B=Bash  R=Read  E=Edit${NC}"
            echo -e "${DIM}                               ${DIM}│${DIM} C=Cached                  ${DIM}│${DIM} C%=Cache hit    ${DIM}│${DIM} S=Son  H=Hai    ${DIM}│${DIM} W=Write T=Task  M=MCP${NC}"
            ;;
