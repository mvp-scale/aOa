...
cmd_domains() {
    ...
    # Column header - competitive ranking format
    printf "${DIM} #  %-26s %5s  %s${NC}\n" "DOMAIN" "HITS" "TERMS"
    ...
    echo "$domains_data" | jq -r '.domains | sort_by(-.hits) | .[]? | "\(.name)|\(.hits // 0)|\(.enriched // false)|\(.terms // [] | .[0:5] | join(" "))"' 2>/dev/null | while IFS='|' read -r name hits enriched terms; do
        ...
        if [ "$enriched" = "true" ] && [ -n "$terms" ]; then
            ...
            printf "${tier_color}%2d${NC}  ${MAGENTA}%-24s${NC} ${DIM}%5s${NC}  ${CYAN}%s${NC}\n" "$rank" "$name_trunc" "$hits_fmt" "$terms"
        else
            printf "${tier_color}%2d${NC}  ${MAGENTA}%-24s${NC} ${DIM}%5s${NC}  ${DIM}-- enrichment required${NC}\n" "$rank" "$name_trunc" "--"
        fi
        ((rank++))
    done
