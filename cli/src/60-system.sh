...
cmd_domains() {
    ...
    case "${1:-}" in
        init)
            ...
            local skipped=$(echo "$result" | jq -r '.skipped // 0')
            local error=$(echo "$result" | jq -r '.error // empty')
            if [ -n "$error" ]; then
                echo -e "${RED}Error: ${error}${NC}" >&2
                return 1
            fi
            if [ "$count" -eq 0 ] && [ "$skipped" -gt 0 ] 2>/dev/null; then
                echo -e "${YELLOW}0 created, ${skipped} skipped (already exist)${NC}"
            elif [ "$skipped" -gt 0 ] 2>/dev/null; then
                echo -e "${GREEN}${count} created${NC}, ${skipped} skipped"
            else
                echo "$count"
            fi
            return 0
        ...
        build)
            ...
            if [ "$domain_name" = "--all" ]; then
                ...
                if [ "$count" -gt 0 ]; then
                    ...
                    # Clear enrich jobs queued by init-skeleton (build --all did the work)
                    curl -s -X POST "${INDEX_URL}/jobs/clear" \
                        -H "Content-Type: application/json" \
                        -d "{\"project_id\":\"${project_id}\",\"queue\":\"all\"}" > /dev/null 2>&1
                fi
        ...
        load-intent)
            ...
            ;;
        rebalance)
            # aoa domains rebalance - clear pending, load intent.json, report
            # Single command for the /aoa-rebalance skill
            shift

            # 1. Clear pending flag
            curl -sf -X POST "${INDEX_URL}/domains/haiku-pending" \
                -H "Content-Type: application/json" \
                -d "{\"project_id\": \"${project_id}\", \"clear\": true}" > /dev/null 2>&1

            # 2. Check for intent.json
            local intent_file="${project_root}/.aoa/domains/intent.json"
            if [ ! -f "$intent_file" ]; then
                echo -e "${RED}Error: No intent.json found${NC}" >&2
                return 1
            fi

            # 3. Validate JSON
            if ! jq empty "$intent_file" 2>/dev/null; then
                echo -e "${RED}Error: intent.json is not valid JSON${NC}" >&2
                return 1
            fi
            ...
            # 6. Clear recent bigrams (signal consumed)
            if [ "$count" -gt 0 ]; then
                curl -sf -X POST "${INDEX_URL}/cc/bigrams/clear?project_id=${project_id}" > /dev/null 2>&1
            fi

            echo -e "${GREEN}✓${NC} ${count} domains added"
            [ "$failed" -gt 0 ] && echo -e "${RED}${failed} failed${NC}"
            return 0
            ;;
    esac
