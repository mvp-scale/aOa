...
cmd_domains() {
    ...
    # Usage: aoa domains [--json] [-n LIMIT] [--help]
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            -n|--limit)
                ...
                ;;
            --help|-h)
                echo "Usage: aoa domains [OPTIONS]"
                echo ""
                echo "Show domain learning status and top domains with terms."
                echo ""
                echo "Options:"
                echo "  --json, -j      Output as JSON (for piping to AI agents)"
                echo "  -n, --limit N   Show top N domains (default: 20)"
                echo "  --help, -h      Show this help message"
                echo ""
                echo "Examples:"
                echo "  aoa domains              # Show top 20 domains"
                echo "  aoa domains -n 10        # Show top 10 domains"
                echo "  aoa domains --json       # Output full structure as JSON"
                return 0
                ;;
            *)
                shift
    ...
    # GL-055: Intent-based tuning
    local tune_count=$(echo "$stats" | jq -r '.tune_count // 0')
    local tune_threshold=$(echo "$stats" | jq -r '.tune_threshold // 100')
    local last_tune=$(echo "$stats" | jq -r '.last_tune // 0')
    ...
    # Format tune counter (GL-055: intent-based)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    echo ""
    # GL-059.5: New header format with source counts (legend style)
    local transition_indicator=""
    if [ "$learned_count" -ge 32 ] 2>/dev/null; then
    ...
    # GL-059.5: Include source and state in output
    echo "$domains_data" | jq -r '.domains[]? | "\(.name)|\(.hits)|\(.created // 0)|\(.source // "seeded")|\(.state // "active")|\(.terms // [] | .[0:5] | join(" "))"' 2>/dev/null | while IFS='|' read -r name hits created source state terms; do
        local hits_fmt
        [ "$hits" -ge 1000 ] && hits_fmt=$(awk "BEGIN {printf \"%.1fk\", $hits/1000}") || hits_fmt="$hits"

        # GL-059.5: Source indicator (- = seeded, + = learned by aOa)
        local src_ind=""
        ...
        local name_trunc="${name:0:20}"

        # GL-059.5: Style based on state
        local name_style="${MAGENTA}"
        local terms_style="${CYAN}"
        if [ "$state" = "stale" ]; then
            name_style="${DIM}"
            terms_style="${DIM}"
        elif [ "$state" = "deprecated" ]; then
            name_style="${DIM}\033[9m"  # Strikethrough
            terms_style="${DIM}"
        fi
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # Dynamic Intent Section (GL-069.1: Real-time semantic tags from hooks)
    # ─────────────────────────────────────────────────────────────────────────

    # Fetch orphan tags - these are the semantic per-prompt tags generated by GL-069.1
    local project_param=""
    [ -n "$project_id" ] && project_param="project_id=${project_id}"
    local orphan_data=$(curl -s "${INDEX_URL}/domains/orphans?${project_param}" 2>/dev/null)

    # Extract orphan tags (no # prefix, single space separator)
    local dynamic_tags=$(echo "$orphan_data" | jq -r '
        .orphans // [] | .[0:10] | join(" ")
    ' 2>/dev/null)
