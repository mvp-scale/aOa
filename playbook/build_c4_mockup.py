...
const MODEL=await fetch(MQ.get("model")||"archmodel.json").then(r=>r.json());
function showFatal(msg){let d=document.getElementById("fatal");if(!d){d=document.createElement("div");d.id="fatal";
  ...
  document.body.appendChild(d);}d.textContent="RENDER FAILURE · "+msg;}
...
function dynamicCatalog(sv){const groups={};
  Object.entries(sv.views).forEach(([id,v])=>{const fam=VIEWFAM[id]||["Views",v.title];
    (groups[fam[0]]=groups[fam[0]]||[]).push({id,label:fam[1],
...
function BoxNode({data}){
  ...
  return html`<div style=${{background:T.card,border:`1.5px ${mock?"dashed":"solid"} ${col}`,
    borderRadius:t==="store"?18:t==="proc"?12:8,padding:"9px 13px",width:data.w-2,height:data.h-2,boxSizing:"border-box",
    color:T.text,opacity:mock?0.88:1,cursor:drill?"pointer":"default"}}>
    ...
    <div style=${{display:"flex",alignItems:"center",gap:8}}>
      ...
      <span style=${{fontSize:13.5,fontWeight:600}}>${data.label}</span>
    ...
    ${data.sub?html`<div style=${{fontSize:10.5,color:T.dim,marginTop:2,marginLeft:25}}>${data.sub}</div>`:null}
...
function EntityNode({data}){
  const c=T.green;
  return html`<div style=${{background:T.card,border:`1.5px solid ${c}`,borderRadius:6,
    width:data.w-2,boxSizing:"border-box",color:T.text,overflow:"hidden"}}>
    <${Handle} type="target" position=${Position.Top} style=${{opacity:0}}/><${Handle} type="target" position=${Position.Left} style=${{opacity:0}}/>
    <div style=${{padding:"7px 12px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:7,background:T.band}}>
      ...
      <span style=${{marginLeft:"auto",fontSize:9.5,color:T.dim}}>${data.tech}</span>
    </div>
...
function SoloNode({data}){const c=data.col;const dash=data.layer==="supporting";
  return html`<div style=${{background:T.band,border:`1.5px ${dash?"dashed":"solid"} ${c}`,
    borderRadius:4,padding:"8px 12px",width:data.w-2,height:data.h-2,boxSizing:"border-box",color:T.text}}>
    <${Handle} type="target" position=${Position.Top} style=${{opacity:0}}/><${Handle} type="target" position=${Position.Left} style=${{opacity:0}}/>
    <div style=${{display:"flex",alignItems:"center",gap:7}}>
      <${Ico} k=${data.ico||data.layer} c=${c} s=${14}/>
      <span style=${{fontSize:11,fontWeight:700,color:c,textTransform:"uppercase",letterSpacing:1.1}}>${data.label}</span>
      <span style=${{fontSize:12.5,fontWeight:600,marginLeft:6}}>${data.member.label}</span>
      <span style=${{marginLeft:"auto",fontSize:10,color:T.dim}}>${data.member.sub}</span>
    </div>
    <${Handle} type="source" position=${Position.Bottom} style=${{opacity:0}}/><${Handle} type="source" position=${Position.Right} style=${{opacity:0}}/>
  </div>`;}
function MemberNode({data}){const c=data.col;
  return html`<div style=${{background:T.card,border:`1px solid ${T.border}`,borderLeft:`3px solid ${c}`,
    borderRadius:6,padding:"5px 10px",width:data.w,height:data.h,boxSizing:"border-box",color:T.text}}>
    <div style=${{display:"flex",alignItems:"center",gap:6}}>
      <${Ico} k=${data.lay} c=${c} s=${12}/>
      <span style=${{fontSize:11.5,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>${data.label}</span>
      ${(data.showC||data.showH)?html`<span title=${(data.concerns?data.concerns+" recon findings · ":"")+(data.changed?"touched in last 15 commits · ":"")+data.sub}
        style=${{marginLeft:"auto",display:"flex",gap:3,flexShrink:0}}>
...
async function layoutSimple(view,dir,d){
  ...
  view.nodes.forEach(n=>{
    if(ent){sizes[n.id]={w:264,h:34+n.fields.length*21+2};}
    else {sizes[n.id]={w:n.type==="sys"?248:208,h:n.sub?60:44};}});
  ...
  return {nodes:view.nodes.map(n=>({id:n.id,type:ent?"entity":"box",position:pos[n.id],
      ...
      data:{...n,w:sizes[n.id].w,h:sizes[n.id].h}})),
...
async function layoutBuckets(view,dir,d,ov){
  ...
  B.forEach(b=>{const n=b.members.length;
    b.solo=(n===1);
    if(b.solo){b.w=d.solo[0];b.h=d.solo[1];return;}
    const cols=n<=4?n:Math.min(5,Math.ceil(n/2));
    const rows=Math.ceil(n/cols);
    b.cols=cols;
    b.w=d.px*2+cols*d.iw+(cols-1)*d.gx;
    b.h=d.head+d.py+rows*d.ih+(rows-1)*d.gy+d.py;});
  const g={id:"root",
  ...
  B.forEach(b=>{const bp=pos[b.id];const col=colOf(b.layer);
    ...
    b.members.forEach((m,i)=>{const c2=i%b.cols,row=Math.floor(i/b.cols);
      nodes.push({id:m.id,type:"member",draggable:false,selectable:false,zIndex:10,
        width:d.iw,height:d.ih,
        position:{x:bp.x+d.px+c2*(d.iw+d.gx),y:bp.y+d.head+d.py+row*(d.ih+d.gy)},
        data:{...m,lay:b.layer,col,w:d.iw,h:d.ih,
  ...
  const edges2=view.edges.map(e=>{const r=rfEdge(e,laidById,ee=>bcol[ee.source]||T.dim);
    if(e._viol){r.style={...r.style,stroke:T.red,strokeDasharray:"6 3",opacity:.95};
      ...
      if(r.data.label)r.data.label.text="⚠ "+r.data.label.text;}
    return r;});
  return {nodes,edges:edges2,problems};}
