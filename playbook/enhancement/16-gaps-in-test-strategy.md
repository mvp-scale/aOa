...
|---|-----|------|------|-------------|----------------------|
| T1 | **G0 keystone benchmark** — the ≤+3% build budget is *asserted, never measured*. No build-time benchmark of parse-with-edges vs without exists. **STATUS (F0, 2026-07-02): MEASURED — GREEN.** L19.6 spike on kubernetes @ `efab4fac` (24,194 files, 5+5 runs): p50 Δ −4.51% (verdict doc), −15.53% on fresh re-execution — spike cost lost in run noise, decisively ≤+3%. Verdict: `details/benchmark-verdict-2026-07-02.md`. Re-run required at L19.13 with the T28/T29 hardening below. | 🟥 | 1 | G0 | Benchmark `BuildIndex` before/after the import-extraction concern on a large real repo (not a no-op baseline). **This is the single make-or-break test** — it decides whether E1 rides the always-on parse or falls back to opt-in/async. |
...
| T8 | **`aoa arch` three-mode parity** — like `grep`/`peek`, must give consistent output/exit codes in direct/pipe/daemon modes. No test. | 🟦 | 2 | G3 | Run each `aoa arch *` subcommand in all three modes; assert identical output + exit codes; the unknown-method fallback (`server.go:246`) stays untouched for non-arch methods. |
...
| # | Gap | Type | Base | Goal at risk | What the test must do |
|---|-----|------|------|-------------|----------------------|
| T28 | **T1 harness lacks statistical power for the gate it enforces** — the F0 protocol ran all-baseline then all-spike sequentially; baseline spread was 72.9–91.3s (~±12%) against a +3% gate, so inter-block drift alone exceeds the gate width and a true +3% regression could pass green. The verdict doc also preserved only p50 + range, not the per-run raws. | 🟥 | 1 (L19.13) | G0/G8 | Interleave/randomize A-B arms per iteration (or discard a warm-up run); record the full per-run raw table in the verdict doc; record memory headroom (F0 hardware was 12GB indexing kubernetes — swap-induced variance risk). |
| T29 | **T1 harness never asserts the spike arm did work** — `spikeImportCount` was written but never asserted/logged; the kickoff §6 ">50K imports processed" precondition went unevidenced (the arm was verified real only by an out-of-band panel test: flag ON → 3/3 imports extracted). | 🟧 | 1 (L19.13) | G8 | The benchmark fails unless the spike arm reports extracted-import count > 50K on the pinned corpus (and 0 with the flag off); log the count into the verdict doc. |
