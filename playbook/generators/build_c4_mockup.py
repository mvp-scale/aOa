...
const ElkEdge=memo(function ElkEdge({id,data,style,markerEnd}){
  ...
  return html`<${React.Fragment}>
    ...
    ${data.label?html`<${EdgeLabelRenderer}>
      <div className="nodrag nopan" style=${{position:"absolute",
        ...
        color:T.text,pointerEvents:"all",zIndex:5}}>${data.label.text}</div>
    <//>`:null}
  <//>`;});
...
function BoxNode({data}){
  ...
  return html`<div class=${hvc(data.id)} style=${{background:T.card,border:`1.5px ${mock?"dashed":"solid"} ${col}`,
    ...
    color:T.text,cursor:drill?"pointer":"default"}}>
...
function EntityNode({data}){
  const c=T.green;
  return html`<div class=${hvc(data.id)} style=${{width:data.w-2}}>
...
function BucketNode({data}){const c=data.col;const dash=data.layer==="supporting"||data.boundary;
  return html`<div style=${{width:"100%",height:"100%",background:T.band,
    ...
    boxShadow:data._god?`0 0 0 2px ${T.red}55, 0 0 22px ${T.red}44`:"none"}}>
    <${Handle} type="target" position=${Position.Top} style=${{opacity:0}}/><${Handle} type="target" position=${Position.Left} style=${{opacity:0}}/>
...
async function layoutSimple(view,dir,d){
  ...
  return {nodes:view.nodes.map(n=>({id:n.id,type:ent?"entity":"box",position:pos[n.id],
    ...
    edges:view.edges.map(e=>{const r=rfEdge(e,laidById,()=>T.dim);
...
async function layoutBuckets(view,dir,d,ov){
  ...
  const edges2=view.edges.map(e=>{const r=rfEdge(e,laidById,ee=>bcol[ee.source]||T.dim);
...
// Bottom dock: the drill tier lives BELOW the canvas, not in the nav rail — clicking an
// element expands this panel upward; the sidebar never reflows. Segments sit side by side.
function BottomDock({sel,setSel,probs,concernsOpen,setConcernsOpen}){
  const showProbs=probs&&probs.length>0&&(concernsOpen||!!sel);
  if(!sel&&!showProbs)return null;
  const Seg=({title,col,children,grow})=>html`<div style=${{flex:grow?2:1,minWidth:0,padding:"10px 16px",
    borderRight:`1px solid ${T.border}`,overflowY:"auto"}}>
    <div style=${{fontSize:9.5,fontWeight:700,letterSpacing:1.2,color:col,marginBottom:7}}>${title}</div>
    ${children}</div>`;
  return html`<div style=${{borderTop:`1px solid ${T.border}`,background:T.band,height:172,
    display:"flex",flexShrink:0,position:"relative"}}>
    ${sel?html`<${Seg} title="DETAIL" col=${T.blue} grow=${true}>
      <div style=${{fontSize:13.5,fontWeight:700,color:T.text,marginBottom:8}}>${sel.label}</div>
      <div style=${{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:"7px 18px"}}>
        ${(sel.rows||[]).map((r,i)=>html`<div key=${i} style=${{minWidth:0}}>
          <div style=${{fontSize:9,fontWeight:700,letterSpacing:.8,textTransform:"uppercase",color:T.mute}}>${r[0]}</div>
          <div style=${{fontSize:11.5,color:T.text,lineHeight:1.4,marginTop:1}}>${r[1]}</div>
        </div>`)}
      </div>
    <//>`:null}
    ${showProbs?html`<${Seg} title=${"CONCERNS · THIS VIEW · "+probs.length} col=${T.red} grow=${!sel}>
      ${probs.map((p,i)=>html`<div key=${i} style=${{fontSize:11,color:T.dim,lineHeight:1.5,marginBottom:6,
        borderLeft:`2px solid ${T.red}`,paddingLeft:9}}>${p}</div>`)}
    <//>`:null}
    <button onClick=${()=>{setSel(null);setConcernsOpen(false);}} title="Close"
      style=${{position:"absolute",top:7,right:10,background:"transparent",border:"none",
...
function Flow(){
  ...
  useEffect(()=>{let on=true;const d=SP[den];
    setEls(null);setSel(null);setConcernsOpen(false);   // unmount canvas NOW; remount only with the new layout so initial fitView sees the right nodes
  ...
  // click = drill tier: drillTo navigates; anything else opens its record in the bottom dock
  ...
  return html`<div style=${{height:"100vh",display:"flex",flexDirection:"column",background:T.bg,
    ...
    <div style=${{flex:1,display:"flex",minHeight:0}}>
      ...
      <div style=${{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
        ...
        <${BottomDock} sel=${sel} setSel=${setSel} probs=${els&&els.problems||[]}
          concernsOpen=${concernsOpen} setConcernsOpen=${setConcernsOpen}/>
...
.hv .hovercard{display:none;position:absolute;left:0;top:calc(100% + 7px);min-width:200px;max-width:300px;
 ...
 box-shadow:0 10px 28px #000c;pointer-events:none;white-space:normal;text-transform:none;letter-spacing:0}
.hv:hover .hovercard,.hv.force .hovercard{display:block}</style>
...
_intent={vid:{"question":v["question"],"vital":v["vital"],"hover":v["hover"]}
